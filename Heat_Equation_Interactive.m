    ...
    set(fig, 'WindowButtonMotionFcn', @mouseClick);
     ...
     function mouseClick(src, ~)
         ...
         switch seltype
             case 'normal'
                ...
                if x >= 1 && x <= Nx-5 && y >= 1 && y <= Ny-5
                    T(y:y+5, x:x+5) = 1; % "Paint" heat on the grid (flip x-y for correct indexing)
                end
            ...
            case 'extend'
                ...
                % Ensure valid indices (flip y-axis to match MATLAB image coordinates)
                if x >= 1 && x <= Nx-5 && y >= 1 && y <= Ny-5
                    T(y:y+5, x:x+5) = 0; % "Paint" cold on the grid (flip x-y for correct indexing)
                end
        ...
        function restart(src,~)
            ...
            if strcmp(nextseltype,'normal')|strcmp(nextseltype,'open')|strcmp(nextseltype,'extend')
